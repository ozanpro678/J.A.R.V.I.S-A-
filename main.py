...
from actions.homework import solve_homework_pdf, answer_homework_question, get_homework_status
...
TOOL_DECLARATIONS = [
    ...
    {
        "name": "open_homework_menu",
        "description": (
            "PDF tabanli odev yukleme menusunu acar. Kullanici 'JARVIS odev menumu ac', "
            "'odev menusunu ac', 'odev yukleyecegim' gibi bir istek soylediginde kullan."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {}
    ...
    {
        "name": "answer_homework_question",
        "description": (
            "Daha once odev menusunden yuklenip cozulmus PDF odevindeki bir sorunun cevabini verir. "
            "Kullanici 'Test 1 1. sorunun cevabi ne', '3. soru neydi', 'odevdeki su soruyu cevapla' "
            "gibi hazir odevle ilgili soru sordugunda kullan."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "query": {
                    "type": "STRING",
                    "description": "Kullanicinin odev sorusu veya istedigi soru numarasi"
                }
            },
            "required": ["query"]
        }
    },
    {
        ...
        "description": (
            "Yuklenen ve cozulmus odevin hazir olup olmadigini, kac soru bulundugunu ozetler. "
            "Kullanici odev hazir mi, odev durumu ne veya kac soru cozuldu diye sordugunda kullan."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {}
...
class JarvisLive:
    def __init__(self, ui: JarvisUI):
        ...
        self.ui.on_homework_file_selected = self._on_homework_file_selected
        self._paused             = False
    ...
    def _on_homework_file_selected(self, file_path: str):
        self.ui.set_state("THINKING")
        self.ui.write_log("SYS: Odev Gemini ile cozuluyor...")
        try:
            result = solve_homework_pdf(file_path)
            failed = self._result_looks_like_error(result) or "tamamlanamadi" in result.lower()
            if failed:
                self.ui.update_homework_status(result, error=True)
                self.ui.write_log(f"ERR: {result}")
                return

            self.ui.update_homework_status(result, ready=True)
            self.ui.write_log(f"JARVIS: {result}")
            self.ui.play_success_sfx()
            self._announce_homework_ready()
        except Exception as exc:
            message = f"Odev cozulurken hata olustu: {exc}"
            self.ui.update_homework_status(message, error=True)
    ...
    def _announce_homework_ready(self):
        if not self._loop or not self.session:
            return
        try:
            asyncio.run_coroutine_threadsafe(
                self.session.send_client_content(
                    turns={"parts": [{"text": "Ödev analizi tamamlandı. Sadece 'Ödevleriniz hazır.' de."}]},
                    turn_complete=True,
                ),
                self._loop,
            )
    ...
    def _on_text_command(self, text: str):
        ...
        normalized = text.lower().replace("ı", "i").replace("ö", "o").replace("ü", "u")
        if "odev" in normalized and "menu" in normalized and ("ac" in normalized or "aç" in normalized):
            self.ui.open_homework_menu()
            self.ui.write_log("JARVIS: Odev menusu acildi. PDF dosyani yukleyebilirsin.")
            return
        if not self._loop or not self.session:
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        parts.append(
            "\n\n[ODEV MODU]\n"
            "- Kullanici 'JARVIS odev menumu ac', 'odev menusunu ac' veya PDF odev yuklemek istedigini soylerse open_homework_menu kullan.\n"
            "- Kullanici yuklenmis odevdeki belirli bir sorunun cevabini sorarsa answer_homework_question kullan.\n"
            "- Kullanici odev hazir mi veya kac soru cozuldu diye sorarsa get_homework_status kullan.\n"
            "- Ornek: 'Test 1 1. sorunun cevabi ne?' -> answer_homework_question(query='Test 1 1. sorunun cevabi ne?')."
        )
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            ...
            elif name == "open_homework_menu":
                self.ui.open_homework_menu()
                result = "Odev menusu acildi. PDF dosyani yukleyebilirsin."

            elif name == "answer_homework_question":
                r = await loop.run_in_executor(
                    None,
                    lambda: answer_homework_question(
                        args.get("query", ""),
                    ),
                )
                result = r or "Odev cevabi bulunamadi."

            elif name == "get_homework_status":
                r = await loop.run_in_executor(None, get_homework_status)
                result = r or "Odev durumu alinamadi."

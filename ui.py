...
class JarvisUI:
    def __init__(self):
        ...
        self.homework_window = None
        self.homework_status_label = None
        self.homework_file_label = None
        ...
        self.on_homework_file_selected = None
    ...
    def open_homework_menu(self):
        self.root.after(0, self._show_homework_menu)

    def _show_homework_menu(self):
        if self.homework_window and self.homework_window.winfo_exists():
            self.homework_window.deiconify()
            self.homework_window.lift()
            self.homework_window.focus_force()
            return

        win = tk.Toplevel(self.root)
        self.homework_window = win
        win.title("JARVIS Odev Menusu")
        win.configure(bg="#00080d")
        win.geometry("640x360")
        win.minsize(560, 320)
        win.transient(self.root)
        win.lift()
        win.focus_force()
        win.protocol("WM_DELETE_WINDOW", self._close_homework_menu)

        body = tk.Frame(win, bg="#00080d", highlightbackground=C_PRI, highlightthickness=1)
        ...
        tk.Label(
            body,
            text="PDF tabanli odevini yukle; JARVIS tum sorulari Gemini ile cozup kaydeder.",
            fg=C_MID,
            bg="#00080d",
            font=font_body(12),
            wraplength=520,
        ...
        self.homework_file_label = tk.Label(
            body,
            text="Secili dosya yok",
            fg=C_TEXT,
            bg="#00080d",
            font=font_body(11),
            wraplength=520,
        )
        self.homework_file_label.pack(pady=(0, 10))

        self.homework_status_label = tk.Label(
            body,
            text="PDF bekleniyor.",
            fg=C_GOLD,
            bg="#00080d",
        ...
        self.homework_status_label.pack(pady=(0, 20))

        buttons = tk.Frame(body, bg="#00080d")
        buttons.pack(pady=(2, 18))
        tk.Button(
            buttons,
            text="PDF YUKLE",
            command=self._select_homework_pdf,
            bg=C_BG,
            fg=C_PRI,
        ...
        tk.Button(
            buttons,
            text="KAPAT",
            command=self._close_homework_menu,
            bg="#08111a",
            fg=C_DIM,
            activebackground="#10202b",
            activeforeground=C_TEXT,
            font=font_body_bold(13),
            borderwidth=0,
            padx=24,
    ...
    def _select_homework_pdf(self):
        file_path = filedialog.askopenfilename(
            parent=self.homework_window or self.root,
            title="Odev PDF dosyasi sec",
        ...
        if self.homework_file_label:
            self.homework_file_label.configure(text=display)
        self.update_homework_status("PDF alindi. JARVIS cozuyor...")
        self.write_log(f"SYS: Odev PDF'i yuklendi: {display}")

        if self.on_homework_file_selected:
            threading.Thread(
                target=self.on_homework_file_selected,
                args=(file_path,),
                daemon=True,
            ).start()

    def update_homework_status(self, text: str, ready: bool = False, error: bool = False):
        def _apply():
            if self.homework_status_label and self.homework_status_label.winfo_exists():
                color = C_RED if error else C_GREEN if ready else C_GOLD
                self.homework_status_label.configure(text=text, fg=color)
        self.root.after(0, _apply)

    def _close_homework_menu(self):
        if self.homework_window and self.homework_window.winfo_exists():
            self.homework_window.destroy()
        self.homework_window = None
        self.homework_status_label = None
        self.homework_file_label = None
